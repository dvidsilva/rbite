Swapi::App.controllers :user do
    ...
    get :nearby do
        ...
        # }
        @nds  = params[:needs].split(',')
        @needs = []
        @nds.each_with_index do |need,ind|
            #@needs[ind] = /#{need.split}/ # Regexp.new(need.to_s, "i");
            @needs << need  # Regexp.new(need.to_s, "i");
            # TODO read user input and aproximate it to the skills
        end
        @skills = Skill.where(:name => {:$in => @needs } )
        lids = [];
        @skills.each do |skill|
            lids << skill.user_id
        end
        @lc = [params[:location][:coords][:longitude], params[:location][:coords][:latitude] ]
        @users = User.where(:id => {:$in => lids } ).where(:id => {:$ne => params[:currentid] } ).all
        # .nearby(@lc,2)
        {:results => @users , :total => @users.count , :lids => lids, :skills => @skills , :needs => @needs }.to_json
        #@lc.to_json
        #@needs.to_json
    end
